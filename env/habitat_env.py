...
class House(object):
        ...
        def obtain_sensor_spec(sensor_type, sensor_uuid, orientation):
            ...
            if self.enable_noise_obs:
                if sensor_type == "color":
                    ...
                    _spec.noise_model_kwargs = dict(intensity_constant=0.1)
                elif sensor_type == "depth":
    ...
    def reset(self, start_loc=None, goal_loc=None):
        ...
        if start_loc is None or goal_loc is None:
            ...
            # # todo: debug
            # self.start_loc = np.array([-0.15,  0.17,  1.55])
            # self.goal_loc = np.array([-3.67,  0.17, 0.83])
    ...
    def step(self, act):
        ...
        done = self.is_terminal(obs['state'])
